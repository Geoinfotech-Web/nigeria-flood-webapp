"""Purge synthetic readings and seed Cloud SQL with real OpenMeteo/GloFAS only.

Steps:
  1. Start Cloud SQL Auth Proxy
  2. Truncate gauge/met readings + flood_features
  3. Real OpenMeteo / GloFAS ingest (up to 92 days)
  4. Historical flood_features backfill
  5. Refresh gauge_hourly
"""
...
def run_psql(db_user: str, db_password: str, db_name: str, sql: str) -> int:
...
def main() -> int:
    ...
    try:
        ...
        print("\n=== Purge synthetic / prior readings ===")
        rc = run_psql(
            db_user, db_password, db_name,
            "TRUNCATE gauge_readings, met_readings, flood_features;",
        )
        if rc != 0:
            return rc
        ...
        steps = [
            ...
            ("Flood features from real readings", [
                sys.executable, str(ROOT / "flink" / "jobs" / "fast_features_real.py"),
            ]),
        ]
        ...
        print("\nReal-only Cloud SQL seed complete.")
        return 0
    finally:
