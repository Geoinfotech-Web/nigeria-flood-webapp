"""Finish real-data feature backfill after ingest (hourly cadence)."""
...
def main() -> int:
    ...
    try:
        ...
        print("=== Truncate partial flood_features ===")
        if run_psql(db_user, db_password, db_name, "TRUNCATE flood_features;") != 0:
            return 1

        # Retry any gauges/met that timed out earlier
        env = {
            **os.environ,
            "DB_HOST": "127.0.0.1",
            "DB_PORT": PROXY_PORT,
            ...
            "DB_NAME": db_name,
            "MET_HISTORY_DAYS": "92",
            "GAUGE_HISTORY_DAYS": "92",
            "FEATURE_STEP_MIN": "60",
            "PYTHONPATH": str(ROOT / "ingest") + os.pathsep + str(ROOT / "flink" / "jobs"),
        }

        print("=== Retry real ingest for any missing stations ===")
        r = subprocess.run(
        ...
        print("=== Refresh gauge_hourly + counts ===")
        return run_psql(
