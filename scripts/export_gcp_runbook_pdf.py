...
SOURCE = ROOT / "GCP_HOSTING_RUNBOOK.md"
TARGET = ROOT / "GCP_HOSTING_RUNBOOK.pdf"
...
def flush_table(story, table_lines, st):
    ...
    for raw in table_lines:
        ...
        if cols:
            story.append(Paragraph(esc(" | ".join(cols)), st["mono"]))
    table_lines.clear()
...
def build():
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=16 * mm,
        bottomMargin=16 * mm,
        title="GGIS Flood Watch GCP Hosting Runbook",
        author="Cursor",
    )
    doc.build(story)
    print(TARGET)
