...
Features computed:
  ...
  - rolling_rain_3h_mm  : IDW rainfall near gauge over 3 h (k=5, ≤250 km)
  - rolling_rain_24h_mm : IDW rainfall near gauge over 24 h
...
def compute_features(conn, station_id: int, bank_full: float, now: datetime, weight_map: dict) -> dict | None:
    """
    ...
    rain_3h = weighted_rainfall_mm(
        conn, station_id, ts_latest - timedelta(hours=3), ts_latest, weight_map,
    )
    rain_24h = weighted_rainfall_mm(
        conn, station_id, ts_latest - timedelta(hours=24), ts_latest, weight_map,
    )

    soil_moisture = min(1.0, rain_24h / 80.0)
...
def run_standalone(once: bool = False):
    ...
    log.info("IDW weights ready for %d gauges (k≤5, ≤250 km)", len(weight_map))
    ...
    while True:
        ...
        for sid, code, bank_full in stations:
            try:
                f = compute_features(conn, sid, bank_full, now, weight_map)
                if f:
                    features.append(f)
                    log.debug(
                        "  %s → level=%.2fm (%.0f%% bank)  Δ1h=%.3f",
