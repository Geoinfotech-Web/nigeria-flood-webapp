...
async def get_readings(
    ...
    GloFAS ingest is daily, so a 24h window often has only one point. When the
    short window is sparse, pad with earlier daily readings (up to 14 days) so
    the water-level chart still shows a useful series.
    """
    ...
    async with request.app.state.db.acquire() as conn:
        rows = await conn.fetch("""
            ...
            ORDER BY time ASC
        """, station_id, since, now)
        if len(rows) < 3:
            pad_since = now - timedelta(days=14)
            rows = await conn.fetch("""
                SELECT time, water_level_m, flow_rate_m3s
                FROM gauge_readings
                WHERE station_id = $1 AND time >= $2 AND time <= $3
                ORDER BY time ASC
            """, station_id, pad_since, now)
    if not rows:
...
async def get_history(
    ...
    async with request.app.state.db.acquire() as conn:
        rows = await conn.fetch("""
            ...
            FROM gauge_hourly
            WHERE station_id = $1 AND bucket >= $2 AND bucket <= $3
            ORDER BY bucket ASC
        """, station_id, since, now)
